...
### ~~Fix Atom Name Conversion in CIF Export (p -> ')~~ ✅ DONE

**Fixed**: Atom names are now correctly written to CIF format (`O5'`, `C3'`, etc.).

The fix was implemented through the unified backbone atom system:
1. Python atom enums store integer values (not string names)
2. The C writer looks up atom names in ATOM_INFO table using these values
3. ATOM_INFO table contains proper CIF names generated by codegen

Verified: Both sampled and loaded structures output correct CIF atom names.
