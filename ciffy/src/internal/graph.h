...
/**
 * Build canonical Z-matrix using precomputed references from codegen.
 *
 * Atoms are processed in natural order, with references looked up from
 * canonical_refs.h when available. For atoms without canonical references,
 * falls back to searching the bond graph.
 *
 * @param atoms            (n_atoms,) int32 atom types
 * @param sequence         (n_residues,) int32 residue types
 ...
 * @param n_residues       Total number of residues
 * @param n_chains         Number of chains
 * @param bond_offsets     (n_atoms+1,) CSR offsets for bond graph
 * @param bond_neighbors   (n_edges,) CSR neighbor indices
 * @param out_zmatrix      Output: (n_atoms, 4) int64 Z-matrix
 * @param out_dihedral_types Output: (n_atoms,) int8 dihedral types
 * @return                 Number of entries written, or -1 on error
 */
...
/**
 * Build atom-indexed Z-matrix from CSR bond graph.
 *
 * Key difference from build_zmatrix_from_csr: output is in NATURAL ATOM ORDER,
 * not BFS traversal order. This means out_zmatrix[k] corresponds to atom
 * (chain_start + k), eliminating the need for atom<->row index mappings.
 *
 * Algorithm:
 * 1. BFS from root to build parent tree and compute levels
 * 2. For each atom k in natural order:
 *    - dist_ref = parent[k]
 *    - ang_ref = parent[parent[k]]
 *    - dih_ref = parent[parent[parent[k]]] (with fallback search)
 *
 * @param offsets          (n_atoms+1,) CSR offsets
 * @param neighbors        (n_edges,) CSR neighbor indices
 * @param n_atoms          Total number of atoms
 ...
 *
 * Same as build_zmatrix_parallel but outputs in natural atom order.
 *
 * @param offsets          (n_atoms+1,) CSR offsets
 * @param neighbors        (n_edges,) CSR neighbor indices
 * @param n_atoms          Total number of atoms
