...
def generate_bond_patterns_header(
    ...
    # Molecule type constants (must match Molecule enum)
    MOL_PROTEIN = 0
    MOL_RNA = 1
    MOL_DNA = 2
    MOL_HYBRID = 3
    MOL_PROTEIN_D = 4
    MOL_CYCLIC_PEPTIDE = 7
    ...
    lines = [
        ...
        f'#define MOL_PROTEIN {MOL_PROTEIN}',
        f'#define MOL_RNA {MOL_RNA}',
        f'#define MOL_DNA {MOL_DNA}',
        f'#define MOL_HYBRID {MOL_HYBRID}',
        f'#define MOL_PROTEIN_D {MOL_PROTEIN_D}',
        f'#define MOL_CYCLIC_PEPTIDE {MOL_CYCLIC_PEPTIDE}',
        '',
    ]
    ...
    for res_idx, res in enumerate(all_residues):
        ...
        if res.molecule_type in (MOL_RNA, MOL_DNA, MOL_HYBRID):
            # Nucleic acids: O3' -> P linkage
            prev_atom = atom_index.get((primary_cif, "O3'"), 0)
            next_atom = atom_index.get((primary_cif, "P"), 0)
        elif res.molecule_type in (MOL_PROTEIN, MOL_PROTEIN_D, MOL_CYCLIC_PEPTIDE):
