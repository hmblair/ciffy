#!/usr/bin/env python3
"""
Train AtomARModel for all-atom structure generation.
...
def train(
    ...
    """Train AtomARModel with simplified Polymer interface."""
    ...
    config = AtomARModelConfig(
        d_model=d_model,
        num_residue_layers=num_layers,
        num_atom_layers=num_layers,
        num_heads=8,
        dropout=0.1,
    )
    model = AtomARModel(residue_atoms, config).to(device)
...
def sample_structures(model: AtomARModel, output_dir: Path, device: str = "cuda"):
...
def main():
    ...
    parser.add_argument("--max-residues", type=int, default=64)
