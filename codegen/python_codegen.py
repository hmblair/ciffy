...
def generate_dihedral_arrays(
    ...
    _write_validated_python(
        biochem_dir / "_generated_dihedrals.py",
        lines,
        "dihedral arrays"
    )
...
def generate_zmatrix_arrays(
    ...
    _write_validated_python(
        biochem_dir / "_generated_zmatrix.py",
        lines,
        "Z-matrix arrays"
    )
...
def generate_python_atoms(
    ...
    Generates raw data (atoms dict, ideal coords, bonds) with underscore prefix.
    The actual AtomGroups with metadata are created in _generated_residues.py.
    """
    ...
    lines = [
        '"""',
        'Auto-generated atom data for AtomGroup system.',
        'DO NOT EDIT - Generated by ciffy/codegen/generate.py from CCD.',
        '',
        'This file contains raw atom data (atoms dict, ideal coords, bonds).',
        'The actual AtomGroup instances with metadata are in _generated_residues.py.',
        '"""',
        '',
        'import numpy as np',
        '',
    ...
    # Generate per-residue atom data
    sections = [
        ("RNA", Molecule.RNA),
        ("DNA", Molecule.DNA),
    ...
    for section_name, mol_type in sections:
        ...
        for res in residues:
            ...
            # Generate atoms dict (prefixed with underscore)
            lines.append(f'_{res.class_name}_atoms = {{')
            for local_idx, (py_name, global_idx) in enumerate(atoms.items()):
                lines.append(f'    "{py_name}": Atom("{py_name}", {global_idx}, {local_idx}),')
            lines.append('}')
            lines.append('')

            # Add ideal coordinates as numpy array
            if res.ideal_coords and res.atoms:
                ...
                if coords:
                    lines.append(f"_{res.class_name}_ideal = np.array([")
...
def generate_python_residues(
    ...
    Creates AtomGroup instances directly with all metadata (value, molecule_type,
    abbrev, ideal, bonds). Inherits from ResidueBase for static methods.
    """
    ...
    # Build set of residues that have geometry (ideal and bonds)
    residues_with_geometry = set()
    for res in residues_with_atoms:
        if res.ideal_coords and res.bonds:
            residues_with_geometry.add(res.class_name)
    ...
    if atom_class_names:
        ...
        for name in atom_class_names:
            lines.append(f'    _{name}_atoms,')
        lines.append(')')
        # Separate import for geometry (only for residues that have it)
        geom_names = [name for name in atom_class_names if name in residues_with_geometry]
        if geom_names:
            lines.append('from ._generated_atoms import (')
            for name in geom_names:
                lines.append(f'    _{name}_ideal,')
                lines.append(f'    _{name}_bonds,')
            lines.append(')')
