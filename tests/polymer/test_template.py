"""Tests for _template polymer generation from sequences."""
...
class TestFromSequence:
    ...
    def test_polymer_id(self, id_arg, expected_id):
        ...
        if id_arg is None:
            ...
            polymer = template("acgu", id=id_arg)
    ...
    def test_elements_are_atomic_numbers(self):
        ...
        polymer = template("M")  # Methionine - has S for variety
...
class TestFromSequenceIntegration:
    """Integration tests for _template polymers."""
    ...
    def test_can_write_to_cif(self, tmp_path):
        ...
        _template = template("acgu", id="test_rna")
        # Add coordinates to _template
        coords = np.random.randn(_template.size(), 3).astype(np.float32)
        polymer = _template.copy(coordinates=coords)
    ...
    def test_round_trip_copy_coordinates(self, tmp_path):
        ...
        from ciffy import template, load

        polymer = template("acgu", id="test_rna")
        n_atoms = polymer.size()
...
class TestFromSequenceEdgeCases:
    ...
    def test_backend_invalid_raises(self):
        ...
        # template might not validate backend (creates numpy then converts)
        # Check that the result has a valid backend regardless
        p = template("acgu", backend="numpy")
        assert p.backend in ["numpy", "torch"]


class TestFromSequenceMultiChain:
    """Test multi-chain template functionality."""
    ...
    def test_different_length_chains(self):
        ...
        polymer = template(["a", "acgu", "acguacgu"])
...
class TestTerminalAtoms:
    ...
    def test_protein_terminal_atoms(self):
        ...
        polymer = template("AAA")  # 3 alanines
        apr = list(polymer.counts(Scale.RESIDUE))
    ...
    def test_multi_chain_each_has_termini(self):
        ...
        # Two separate chains
        multi = template(["ac", "gu"])

        # Compare to single 4-residue chain
        single = template("acgu")
    ...
    def test_terminal_atoms_per_chain_independent(self):
        ...
        two_singles = template(["a", "a"])

        # One two-residue chain
        one_double = template("aa")
...
class TestTemplateMatchesCIF:
    """Test that template produces structures consistent with CIF files."""
    ...
    def _verify__template_matches_loaded(self, loaded, _template):
        """
        Verify _template matches loaded CIF (except coordinates).

        Checks:
        1. Residue sequences match exactly
        2. Loaded atoms are subset of _template atoms (per residue)
        ...
        # 1. Sequences match
        assert np.array_equal(loaded.sequence, _template.sequence), \
            "Residue sequences differ"

        # 2. Loaded atoms are subset of _template atoms per residue
        loaded_sizes = loaded.counts(Scale.RESIDUE)
        _template_sizes = _template.counts(Scale.RESIDUE)
        ...
        for i in range(len(loaded_sizes)):
            ...
            _template_atoms = set(_template.atoms[_template_offset:_template_offset + _template_sizes[i]].tolist())

            assert loaded_atoms.issubset(_template_atoms), \
                f"Residue {i}: loaded atoms {loaded_atoms - _template_atoms} not in _template"

            loaded_offset += loaded_sizes[i]
            _template_offset += _template_sizes[i]

        # 3. Elements match for corresponding atom types
        _template_atom_to_element = dict(zip(
            _template.atoms.tolist(),
            _template.elements.tolist()
        ))

        for atom_idx, elem in zip(loaded.atoms.tolist(), loaded.elements.tolist()):
            _template_elem = _template_atom_to_element.get(atom_idx)
            if _template_elem is not None:
                assert elem == _template_elem, \
                    f"Element mismatch for atom {atom_idx}: loaded={elem}, _template={_template_elem}"

        # 4. Counts match
        assert loaded.size(Scale.CHAIN) == _template.size(Scale.CHAIN), "Chain counts differ"
        assert loaded.size(Scale.RESIDUE) == _template.size(Scale.RESIDUE), "Residue counts differ"

    def test_1zew_dna_consistency(self):
        """Test _template matches 1ZEW (DNA duplex)."""
        from ciffy import load, template

        loaded = load(get_test_cif("1ZEW"))
        sequences = self._extract_chain_sequences(loaded)
        _template = template(sequences)

        self._verify__template_matches_loaded(loaded, _template)

    def test_9gcm_rna_protein_consistency(self):
        """Test _template matches 9GCM (RNA + protein complex)."""
        from ciffy import load, template
...
class TestBondsAndLinking:
    ...
    # Note: Tests for residue spacing were removed because template()
