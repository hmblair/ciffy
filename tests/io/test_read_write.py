...
class TestLoad:
    ...
    def test_repr_molecule_types(self, cif_file, backend):
        ...
        for line in lines:
            ...
            if not data_started:
                continue
            if line.strip():
                # Skip separator lines and summary line (starts with Σ)
                parts = line.split()
                if not parts or parts[0] in ('─', 'Σ') or line.startswith('─'):
                    continue
    ...
    def test_molecule_type_classification(self, cif_file, backend):
        ...
        rna_subset = polymer.molecule_type(Molecule.RNA)
